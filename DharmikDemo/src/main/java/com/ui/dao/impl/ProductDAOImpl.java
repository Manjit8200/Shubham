...
public class ProductDAOImpl implements ProductDAO {
	...
	public String addProduct(Product m) {
		 logger.info("***** addProduct DAO Impl*****");
	       String sql = "insert into product (category_id,sub_category_id,type_id,product_name,product_code,product_size,product_desc,product_cost,product_margin,product_profit,product_discount, product_selling,product_available,status,created_by,ip_address) values (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)";
       ...
       Connection conn = null;
	       ...
	       try
	       {
	           ...
	           PreparedStatement ps = conn.prepareStatement(sql);
				...
				ps.setInt(3, m.getType_id());
				ps.setString(4, m.getProduct_name());
				ps.setString(5, m.getProduct_code());
				ps.setString(6, m.getProduct_size());
				ps.setString(7, m.getProduct_desc());
				ps.setFloat(8, m.getProduct_cost());
				ps.setFloat(9, m.getProduct_margin());
				ps.setFloat(10, m.getProduct_profit());
				ps.setFloat(11, m.getProduct_discount());
				ps.setFloat(12, m.getProduct_selling());
				ps.setInt(13, m.getProduct_available());
				ps.setString(14, m.getStatus());
				ps.setInt(15, m.getCreatedBy());
				ps.setString(16, m.getIpAddress());
	...
	public String addProductTaxData(Product m1) {
		 logger.info("***** addProductTaxData DAO Impl*****");
	     	...
	     	String sql = "insert into product_image (product_image,status,ip_address) values (?,?,?)";
			        Connection conn = null;
			        ...
			        try
			        {
						...
						/*
						 * // ps.setInt(1, m1.getProduct_id()); ps.setInt(2, m1.getTax_type());
						 * ps.setFloat(3, m1.getTax_value());
						 */		   
			            ps.setString(1, m1.getProduct_image());
				        ps.setString(2, m1.getStatus());
						
						ps.setString(3, m1.getIpAddress());      
	...
	@Override
	public List<Product> getAllProductImages() {
		  logger.info("+++++ GET getAllProductImages +++++");
		    List<Product> mmp = new ArrayList<Product>();
		    String s = "y";
		    String sql = "SELECT * FROM product_image where status=?";
		    Connection conn = null;
		    try {
		        conn = dataSource.getConnection();
		        PreparedStatement ps = conn.prepareStatement(sql);
		        ...
		        while (rs.next()) {
		              ...
		              m.setProduct_image(rs.getString("product_image")); 	
			      ...
			      int id=0;
			        ...
			        while (rs.next()) {
				     ...
				     id= rs.getInt("product_id");
			        	...
			        	System.out.println("/////////////////////product DAO////////////////////////////"+id);
			        }
			        rs.close();
			        ps.close();
			        return id;
	...
	@Override
	public String addProductImageData(Product m1) {
		 logger.info("***** addProductImageData DAO Impl*****");
	   	
	     	String sql = "insert into product_image (product_id,product_sequence,product_image,barcode_image,qrcode_image,status,created_by,ip_address,Secertcode) values (?,?,?,?,?,?,?,?,?)";
			        Connection conn = null;
			        ...
			        try
			        {
					   ...
					   System.out.println("+++++++++++++++++++>>>>>>>>>>>>+++++="+m1.getProduct_id());
			            ...
			            ps.setInt(2, m1.getProduct_sequence());
			            ps.setString(3, m1.getProduct_image());
			            ps.setString(4, m1.getBarcodeImage());
			            ps.setString(5, m1.getQrcodeimage());
				        ps.setString(6, m1.getStatus());
				    	ps.setInt(7, m1.getCreatedBy());
						ps.setString(8, m1.getIpAddress());    
						ps.setString(9, m1.getSecertcode());
